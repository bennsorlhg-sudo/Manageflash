    ...
    <View style={[styles.container, { paddingTop: Platform.OS === "web" ? 12 : insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.userName}>{data?.ownerName ?? "فهد الهندي"}</Text>
        </View>
      ...
      <ScrollView
        style={styles.scroll}
        contentContainerStyle={[styles.scrollContent, { paddingBottom: Platform.OS === "web" ? 100 : insets.bottom + 100 }]}
      ...
      >
        {loading ? (
          ...
          <>
            ...
            {/* Summary Cards — 2×2 full-width grid */}
            <View style={styles.gridRow}>
              <SummaryCard
                icon="layers"
                label="إجمالي العهدة"
                value={fmt(data?.totalCustody ?? 0)}
                color={Colors.warning}
              />
              <SummaryCard
                icon="trending-up"
                label="إجمالي السلف"
                value={fmt(data?.totalLoans ?? 0)}
                color={Colors.error}
              />
            </View>
            <View style={styles.gridRow}>
              <SummaryCard
                icon="card"
                label="إجمالي الكروت"
                value={fmt(data?.totalCardValue ?? 0)}
                color={Colors.success}
              />
              <SummaryCard
                icon="storefront"
                label="نقاط البيع"
                value={String(data?.totalSalesPoints ?? 0)}
                color={Colors.roles.supervisor}
              />
            </View>

            {/* Network Stats — separate full-width cards side by side */}
            <Text style={styles.sectionTitle}>الشبكة</Text>
            <View style={styles.gridRow}>
              <View style={[styles.networkStatCard, { borderBottomColor: Colors.primary }]}>
                <Ionicons name="wifi" size={28} color={Colors.primary} />
                <Text style={styles.networkStatValue}>{data?.hotspotCount ?? 0}</Text>
                <Text style={styles.networkStatLabel}>هوتسبوت</Text>
              </View>
              <View style={[styles.networkStatCard, { borderBottomColor: Colors.success }]}>
                <Ionicons name="radio" size={28} color={Colors.success} />
                <Text style={styles.networkStatValue}>{data?.broadbandCount ?? 0}</Text>
                <Text style={styles.networkStatLabel}>برودباند</Text>
              </View>
            </View>
      ...
      <AddCustodyModal
        ...
        onSuccess={() => { setShowCustodyModal(false); fetchDashboard(); }}
        insets={insets}
      />
      <AddTaskModal
...
function AddCustodyModal({ visible, token, onClose, onSuccess, insets }: {
  visible: boolean; token: string | null; onClose: () => void; onSuccess: () => void; insets: any;
}) {
              ...
              <View style={styles.fieldGroup}>
                ...
                <View style={styles.segmented}>
                  {[{ v: "finance_manager", l: "المسؤول المالي" }, { v: "supervisor", l: "المشرف" }].map((opt) => (
                    <TouchableOpacity
                      key={opt.v}
                      style={[styles.segBtn, toRole === opt.v && styles.segBtnActive]}
                      onPress={() => setToRole(opt.v as any)}
                    >
...
function AddTaskModal({ visible, token, onClose, onSuccess, insets }: {
  visible: boolean; token: string | null; onClose: () => void; onSuccess: () => void; insets: any;
}) {
            ...
            <View style={styles.fieldGroup}>
              ...
              <View style={styles.roleOptions}>
                {roles.map((r) => (
                  <TouchableOpacity
                    key={r.v}
                    style={[styles.roleOption, targetRole === r.v && styles.roleOptionActive]}
                    onPress={() => setTargetRole(r.v as any)}
                  >
...
const styles = StyleSheet.create({
  ...
  userName: { fontSize: 22, color: Colors.text, fontFamily: "Inter_700Bold", textAlign: "right" },
  logoMini: {
  ...
  scroll: { flex: 1 },
  scrollContent: { paddingHorizontal: 12, paddingTop: 14, gap: 14 },
  ...
  primaryCardInner: { flexDirection: "row-reverse", alignItems: "center", gap: 16 },
  primaryLabel: { fontSize: 13, fontFamily: "Inter_400Regular", color: Colors.textSecondary, textAlign: "right", marginBottom: 8 },
  primaryValue: { fontSize: 34, fontFamily: "Inter_700Bold", color: Colors.primary, textAlign: "right" },
  primaryIcon: {
  ...
  summaryLabel: { fontSize: 12, fontFamily: "Inter_400Regular", color: Colors.textSecondary, textAlign: "right" },
  ...
  networkStatValue: { fontSize: 30, fontFamily: "Inter_700Bold", color: Colors.text },
  networkStatLabel: { fontSize: 13, fontFamily: "Inter_500Medium", color: Colors.textSecondary },
  ...
  actionLabel: { fontSize: 14, fontFamily: "Inter_600SemiBold", color: Colors.text, textAlign: "center" },
  ...
  reportLabel: { fontSize: 13, fontFamily: "Inter_600SemiBold", color: Colors.text },
  ...
  segBtn: { flex: 1, paddingVertical: 11, alignItems: "center", backgroundColor: Colors.inputBackground },
